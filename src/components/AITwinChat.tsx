      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-[22rem] max-w-[calc(100vw-3rem)] bg-background/95 dark:bg-background/95 backdrop-blur-2xl border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl shadow-black/20 flex flex-col overflow-hidden"
            style={{ height: "480px" }}
          >
            {/* Header */}
                  ...
                  <div
                    className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${
                      msg.role === "user"
                        ...
                        : "bg-black/5 dark:bg-white/[0.06] border border-black/5 dark:border-white/5 rounded-bl-md"
                    }`}
                  >
                    {msg.content}
                  </div>
              ...
              <div className="px-4 pb-2 flex flex-wrap gap-1.5">
                {chatSuggestions.map((s) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-full text-[10px] font-medium bg-black/5 dark:bg-white/[0.04] border border-black/5 dark:border-white/5 text-muted-foreground hover:text-foreground hover:bg-black/10 dark:hover:bg-white/10 transition-all duration-200 active:scale-95 text-left"
                  >
                    {s}
                  </button>
                ))}
